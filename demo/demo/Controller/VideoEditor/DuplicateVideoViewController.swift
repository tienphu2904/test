...
class DuplicateVideoViewController: AssetSelectionVideoViewController {
    ...
    @IBAction func save(_ sender: Any) {
        ...
        if quality == "None" {
            if st == 0 {
                let url1 = createUrlInApp(name: "1.MOV")
                removeFileIfExists(fileURL: url1)
                let cut1 = "-ss \(zero) -i \(filePath) -to \(tl) -c copy \(url1)"
                MobileFFmpeg.execute(cut1)
                
                let url2 = createUrlInApp(name: "2.MOV")
                removeFileIfExists(fileURL: url2)
                let cut2 = "-ss \(st1) -i \(filePath) -to \(end) -c copy \(url2)"
                MobileFFmpeg.execute(cut2)
                print(url2)
                
                let url3 = createUrlInApp(name: "3.MOV")
                removeFileIfExists(fileURL: url3)
                let cut3 = "-i \(url1) -i \(url1) -i \(url2) -filter_complex \"[0:v:0] [0:a:0] [1:v:0] [1:a:0] [2:v:0] [2:a:0] concat=n=3:v=1:a=1 [v] [a]\" -map \"[v]\" -map \"[a]\" \(url3)"
                ...
                let url3 = createUrlInApp(name: "3.MOV")
                removeFileIfExists(fileURL: url3)
                let cut3 = "-i \(url2) -i \(url1) -i \(url1) -filter_complex \"[0:v:0] [0:a:0] [1:v:0] [1:a:0] [2:v:0] [2:a:0] concat=n=3:v=1:a=1 [v] [a]\" -map \"[v]\" -map \"[a]\" \(url3)"
                DispatchQueue.main.async {
                    ZKProgressHUD.show()
                }
                let serialQueue = DispatchQueue(label: "serialQueue")
...
//                let asss = AVAsset(url: url3)
//                loadAsset(asss)
//                print(url3)
            } else {
                let url = createUrlInApp(name: "a.MOV")
                ...
                let url3 = createUrlInApp(name: "c.MOV")
                removeFileIfExists(fileURL: url3)
                let cut3 = "-i \(url) -i \(url1) -i \(url1) -i \(url2) -filter_complex \"[0:v:0] [0:a:0] [1:v:0] [1:a:0] [2:v:0] [2:a:0] [3:v:0] [3:a:0] concat=n=4:v=1:a=1 [v] [a]\" -map \"[v]\" -map \"[a]\" \(url3)"
                DispatchQueue.main.async {
                    ZKProgressHUD.show()
                }
                let serialQueue = DispatchQueue(label: "serialQueue")
            ...
            let url = createUrlInApp(name: "vd1.MOV")
            removeFileIfExists(fileURL: url)
            let cut = "-ss \(st) -i \(filePath) -to \(tl) -c copy \(url)"
            MobileFFmpeg.execute(cut)
            
            let url1 = createUrlInApp(name: "vd2.MOV")
            removeFileIfExists(fileURL: url1)
            let cut1 = "-ss \(zero) -i \(filePath) -to \(crtime) -c copy \(url1)"
            MobileFFmpeg.execute(cut1)

            let url2 = createUrlInApp(name: "vd3.MOV")
            removeFileIfExists(fileURL: url2)
            let cut2 = "-ss \(crtime) -i \(filePath) -to \(dr) -c copy \(url2)"
            MobileFFmpeg.execute(cut2)
        
            let furl = createUrlInApp(name: "vd4.MOV")
            removeFileIfExists(fileURL: furl)
            let furl1 = createUrlInApp(name: "vd44.MOV")
            ...
            if quality == "PushRight"{
                ...
                let furl1 = createUrlInApp(name: "vd5.MOV")
                removeFileIfExists(fileURL: furl1)
                ...
                let furl2 = createUrlInApp(name: "vd5.MOV")
                removeFileIfExists(fileURL: furl2)
                let cmdvd4 = "-i \(furl1) -i \(audio2) -c copy -map 0:v -map 1:a \(furl2)"
                DispatchQueue.main.async {
                    ZKProgressHUD.show()
                }
                let serialQueue = DispatchQueue(label: "serialQueue")
                serialQueue.async {
                    if self.quality == "CrossFade"{
                        ...
                        MobileFFmpeg.execute(ad2)
                        MobileFFmpeg.execute(cmdvd4)
                        CustomPhotoAlbum.sharedInstance.saveVideo(url: furl2)
                    }
